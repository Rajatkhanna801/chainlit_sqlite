...
from literalai.thread import Thread as Literal_Thread
...
class SQLiteClient:
    def __init__(self, db_filepath=None):
        if db_filepath is None or db_filepath == "":
            db_filepath = "user_database/chainlit.db"
...
class API:
    ...
    def normalize_db_path(self, db_path):
        abs_path = os.path.abspath(db_path)
        normalized_path = os.path.normpath(abs_path)
        if os.name == "nt":
            normalized_path = normalized_path.replace("\\", "/")
    ...
    async def get_thread(self, id):
        ...
        if thread_steps:
            for step in thread_steps:
                ...
                step_data = {
                    ...
                    'startTime': step.start_time.isoformat() or "", 
                    'endTime': step.end_time.isoformat() or "", 
                    'createdAt': step.created_at.isoformat() or "", 
                    'type': step.type, 
                    'error': None, 
                    'input': step.input, 
                    'output': step.output, 
        ...
        return Literal_Thread.from_dict(thread_dict) if thread_dict else None
